sap.ui.define([
	...
	"use strict";
	var that;
	return BaseController.extend("Trade_Finance.Trade_Finance.controller.bbankreviewedviewDeatils", {
		...
		onInit: function () {
			...
			that = this;
			this.setModel(oViewModel, "buyerbankSubmitView");
			this.getRouter().getRoute("bbankreviewedviewDeatils").attachMatched(this.onBuyerbankSubmittedMatched, this);

		},

		onBuyerbankSubmittedMatched: function (oEvent) {
			var clear = "";
			...
			this.getModel("BuyerbankSubmitModel").loadData(sBuyerSmittigUri,
				null, true, "GET", null,
				false, {
				...
				that.getView().getModel("buyerbankSubmitView").setProperty("/token", that.getModel("BuyerbankSubmitModel").getProperty(
					"/data/LCTokenNumber"));
				...
				var url = "https://blockchaintradefinance.cfapps.eu10.hana.ondemand.com/api/lcHistory/getLcHistoryByLcToken/" + that.getModel(
					"BuyerbankSubmitModel").getProperty("/data/LCTokenNumber");
				jQuery.ajax({
					type: "GET",
		...
		bbankreviewviewdeatils: function () {
