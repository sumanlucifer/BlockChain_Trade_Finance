sap.ui.define([
	...
	"use strict";
	var that;
	return BaseController.extend("Trade_Finance.Trade_Finance.controller.buyerBank", {
		formatter: formatter,
		onInit: function () {
		...
		loadData: function () {
			...
			$.ajax({
				type: "GET",
				url: "https://blockchaintradefinance.cfapps.eu10.hana.ondemand.com/api/landingPage?newlc=SUB_BUYER&status=APP_BUYER_BANK,REJ_BUYER_BANK,APP_SELLER_BANK,REJ_SELLER_BANK,GOODS_DIS_PEN,REJ_SELLER,PAYMENT_PROGRESS,COMPLETED",
				headers: {
					"Content-Type": "application/json",
					"Accept": "application/json",
					"auth-token": that.getOwnerComponent().oMyStorage.get("data").token,
				...
				success: function (odata) {
					...
					that.getView().getModel("buyerlccreateModel").setProperty("/SubmittedByBuyer", data.statistics.SUB_BUYER);
					that.getView().getModel("buyerlccreateModel").setProperty("/submitList", (data.statistics.APP_BUYER_BANK +
					...
					that.getView().getModel("buyerlccreateModel").setProperty("/submittedlcs", data.newLC);
		...
		/* open Rjectfragment*/
		onRejectDialogPress: function (oEvent) {
		...
		newReiwedDetails: function (oEvent) {
			...
			this.getRouter().navTo("bbankreviewedviewDeatils", {
				tokenPath: oTokenPathID,
				sLCId: oBindingContext.getProperty("LCId")
			});
		},
		...
		searchBuyer: function (oEvent) {
			...
			var oFilterName = new sap.ui.model.Filter([
				new sap.ui.model.Filter("LCStatus", sap.ui.model.FilterOperator.EQ, "SUB_BUYER")
			]);
		...
		searchBuyerSubmit: function (oEvent) {
			...
			var oFilterName = new sap.ui.model.Filter([
				new sap.ui.model.Filter("LCStatus", sap.ui.model.FilterOperator.NE, "SUB_BUYER")
			]);
			var andFilter = new sap.ui.model.Filter([filters, oFilterName], true);
			var aFilters = [];
			aFilters.push(andFilter);
