sap.ui.define([
	...
	"use strict";
	var that;
	return BaseController.extend("Trade_Finance.Trade_Finance.controller.newlcsellerviewDetails", {
		...
		onInit: function () {
			...
			that = this;
			this.setModel(oViewModel, "sellerPendingView");
			this.getRouter().getRoute("newlcsellerviewDetails").attachMatched(this.onSellerSubmittedMatched, this);

		},

		onSellerSubmittedMatched: function (oEvent) {
			var clear = "";
			...
			this.sowneraddress = oArgs.ownerAdd;

			this.getModel("SellerSubmittedModel").loadData(sBuyerPendingUri,
				null, true, "GET", null,
				false, {
				...
				that.getView().getModel("sellerPendingView").setProperty("/token", that.getModel("SellerSubmittedModel").getProperty(
					"/data/LCTokenNumber"));
				...
				var url = "https://blockchaintradefinance.cfapps.eu10.hana.ondemand.com/api/lcHistory/getLcHistoryByLcToken/" + that.getModel(
					"SellerSubmittedModel").getProperty("/data/LCTokenNumber");
				jQuery.ajax({
					type: "GET",
		...
		/* sellerPending view details submit functionality*/
		SellerViewApprove: function () {

			var oPayload = {

				"fromAddress": this.sowneraddress,

				"toAddress": this.getOwnerComponent().oMyStorage.get("data").address,

				"LcTokenId": this.sLCid,
			...
			jQuery.ajax({
				...
				success: function (odata) {
					...
					that.getRouter().navTo("sellerlanding");
					sap.ui.core.BusyIndicator.hide();
		...
		// 		this.oapproveDialog = new sap.ui.xmlfragment("finalcreatsubmitFrag", "Trade_Finance.Trade_Finance.fragment.approve",
		// 			this);
		 ...
		 * @memberOf Trade_Finance.Trade_Finance.view.newlcsellerviewDetails
		 */
