sap.ui.define([
	...
	"use strict";
	var that;
	return BaseController.extend("Trade_Finance.Trade_Finance.controller.newlcsellerbankviewDetails", {
		...
		onInit: function () {
			...
			that = this;
			this.setModel(oViewModel, "sellerbankPendingView");
			this.getRouter().getRoute("newlcsellerbankviewDetails").attachMatched(this.onSellerBankSubmittedMatched, this);
		...
		onSellerBankSubmittedMatched: function (oEvent) {
			...
			this.sBowneraddress = oArgs.ownerAdd;

			this.getModel("SellerBankSubmittedModel").loadData(sBuyerPendingUri,
				null, true, "GET", null,
				false, {
				...
				that.getView().getModel("sellerbankPendingView").setProperty("/token", that.getModel("SellerBankSubmittedModel").getProperty(
					"/data/LCTokenNumber"));
		...
		sellerbankapproveback: function () {
			that.getRouter().navTo("sellerBnaklandingPage");

		},

		/* sellerbankPending view details submit functionality*/
		SellerBankViewApprove: function () {

			var oPayload = {

				"fromAddress": this.sBowneraddress,
		 ...
		 * @memberOf Trade_Finance.Trade_Finance.view.newlcsellerbankviewDetails
		 */
